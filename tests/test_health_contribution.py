...
def test_health_contribution_monthly_uses_previous_month_income_by_default():
    ...
    assert summary.monthly["2026-02"].business_income == Decimal("0.00")
    assert summary.monthly["2026-02"].contribution == Decimal("324.41")
    assert summary.total == Decimal("639.37")


def test_health_contribution_monthly_can_use_current_month_income():
    summary = calculate_health_contribution_monthly(
        monthly_jpk={
            "2026-01": {
                "sales_net": Decimal("1000.00"),
...
def test_health_contribution_monthly_can_use_previous_month_income_explicitly():
    summary = calculate_health_contribution_monthly(
        monthly_jpk={
            "2026-01": {
                "sales_net": Decimal("10000.00"),
    ...
    assert summary.monthly["2026-01"].contribution == Decimal("314.96")
    assert summary.monthly["2026-02"].business_income == Decimal("9000.00")
    assert summary.monthly["2026-02"].contribution == Decimal("810.00")
